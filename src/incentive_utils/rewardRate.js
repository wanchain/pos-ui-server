...
class RewardRate {
  ...
  estimateMinerRewardWithDelegator(amount, locktime, delegateAmount, feeRate) {
    //Because the first epoch do not generate block, the last epoch do not send message. so it is locktime - 1;
    //locktime = locktime - 1; 
    ...
    let sumSelfProb = 0
    let prob = Number(this.web3.fromWei(this.web3.pos.calProbability(amount, locktime)))
    let delegateProb = Number(this.web3.fromWei(this.web3.pos.calProbability(delegateAmount, 7)))
    sumSelfProb = prob + delegateProb

    let totalReward = total * locktime * (sumSelfProb / (sumProbility + sumSelfProb))
    
    let feeValue = totalReward * Number(feeRate) / 100.0

    totalReward -= feeValue

    totalReward = totalReward * amount / (amount + delegateAmount) + feeValue;
